...
@Component({
  ...
  styleUrls: ["./resources-detail.component.sass"],
})
export class ResourcesDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // brodcast data for login user
    this.accountData = "";
    this.UserService.castAccountDataList.subscribe((accountData) => {
  ...
  get_news_details(event?: any, limitChanged?: any) {
    ...
    // get news details
    this.ResourcesService.getResourcesDetails(this.cond).subscribe(
      (res) => {
        if (res && res.data) {
          ...
          let ogurl = location.origin + "/resource/" + this.newsdetail.slug;
          this.ogurl = ogurl;
          let ogimage =
          ...
          window["__sharethis__"].load("inline-share-buttons", {
            url: location.origin + "/details/" + this.newsdetail.slug,
          });
        }
      },
      (err) => {
  ...
  getNewsComments(NewsID) {
    if (NewsID) {
      ...
      this.NewsComments = [];
      // Calling service
      this.ResourcesService.getResourceComments(cond).subscribe(
        (res) => {
          if (res && res.data) {
