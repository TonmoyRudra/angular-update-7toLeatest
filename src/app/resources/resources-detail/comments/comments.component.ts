...
@Component({
  selector: 'app-resources-comments',
  templateUrl: './comments.component.html',
  styleUrls: ['./comments.component.sass']
})
export class ResourcesCommentsComponent implements OnInit {
  ...
  onSubmit(frmData) {
    ...
    if (this.commentForm.valid) {
      ...
      this.ResourcesService.SaveResourcePostComment(frmData).subscribe((res) => {
        // show message
  ...
  getNewsComments() {
    if (this.NewsID) {
      ...
      // Calling service
      this.ResourcesService.getResourceComments(cond).subscribe(res => {
        if (res && res.data) {
